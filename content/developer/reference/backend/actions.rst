    ...
    def _cron_do_something(self):
        ...
        with open_some_connection() as conn:
            for record in records:
                ...
                # lock_records: record may disappear while after committing,
                #     break prefetch and we can lock for update
                # filtered_domain: record may have changed
                record = record.lock_records(strict=False).filtered_domain(domain)
                if not record:
                    continue
                try
